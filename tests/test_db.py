...
async def test_generos(db):
    ...
    g1 = await db.upsert_genero("Action", nome_pt="Ação")
    g2 = await db.upsert_genero("Comedy", nome_pt="Comédia")
    ...
    nomes = {g["nome"] for g in genres}
    nomes_pt = {g["nome_pt"] for g in genres}
    assert "Action" in nomes
    assert "Comedy" in nomes
    assert "Ação" in nomes_pt
    assert "Comédia" in nomes_pt


async def test_genero_sem_traducao(db):
    """Genero sem nome_pt deve retornar o original em ambos os campos."""
    g_id = await db.upsert_genero("Unknown Genre")
    anime_id = await db.upsert_anime({"slug": "test", "tipo": "legendado"})
    await db.link_anime_genero(anime_id, g_id)

    genres = await db.get_generos_by_slug("test")
    assert genres[0]["nome"] == "Unknown Genre"
    assert genres[0]["nome_pt"] == "Unknown Genre"  # fallback
